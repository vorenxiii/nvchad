...
function M.hexToHSL(hex)
  ...
  local hsla = string.format('hsla(%d, %d%%, %d%%, 1)', math.floor(h + 0.5), math.floor(s + 0.5), math.floor(l + 0.5))

  -- Adding '%' after second and third argument since it didn't work in the format above
  -- Split the input string into substrings separated by commas
  local parts = {}
  for part in hsla:gmatch '[^,]+' do
    table.insert(parts, part)
  end

  -- Insert '%' sign after the second and third values
  parts[2] = parts[2]:gsub('(%d+)', '%1%%')
  parts[3] = parts[3]:gsub('(%d+)', '%1%%')

  -- Concatenate the modified parts back into a string
  local hsla_reformed = table.concat(parts, ',')

  return hsla_reformed
end
